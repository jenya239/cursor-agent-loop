...
INTERVAL="${CR_OVERNIGHT_INTERVAL_SEC:-300}"
PIDFILE="${HOME}/.cursor/cr-overnight-loop.pid"
LOG="${CR_OVERNIGHT_LOG:-${HOME}/.cursor/cr-overnight.log}"
LOCKFILE="${HOME}/.cursor/cr-overnight-loop.lock"
...
while true; do
  ...
  # Guard errors go to a tmp file so we can log them structured, not as "tick failed"
  GUARDERR=$(mktemp)
  if ! npx tsx scripts/overnight-guard.ts 2>"$GUARDERR"; then
