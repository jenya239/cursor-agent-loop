...
import { PROBE_COMPOSER_PANEL_JS, parseComposerPanelProbe } from '../src/cdp/composer-panel';
    ...
    try {
      ...
      try {
        ...
        const r = (await send('Runtime.evaluate', {
          expression: PROBE_COMPOSER_PANEL_JS,
          returnByValue: true,
        })) as { result?: { value?: unknown } };
        const panel = parseComposerPanelProbe(r.result?.value);
        console.log(JSON.stringify({ window: page.title, panel }, null, 2));
      } finally {
        close();
      }
    } catch (e) {
